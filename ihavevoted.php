<?php
   require_once 'db.php';
   $sql  = "select distinct fprmk from msg where id='".$_SESSION['id']."'";
   $result =$db->query($sql);
   $num=$result->numRows();
   ...
   if ($num>0){
      ...
      $sql  = "select bdate,applyid,title,rate,sdate,num,prmk from vote where prmk in ($idstr) order by prmk";
      $result =$db->query($sql);
      while ($row=$result->fetchRow()){
         $bdate=$row[0];
         $applyid=$row[1];
...
<nav class="navbar navbar-expand-md navbar-dark fixed-top bg-dark">
  <div class="container-fluid aabc">
	...
	<li class="nav-item active">
          <a class="nav-link" aria-current="page" href="addnew.php">&nbsp;新增投票&nbsp;</a>
	</li>
	<li class="nav-item active">
